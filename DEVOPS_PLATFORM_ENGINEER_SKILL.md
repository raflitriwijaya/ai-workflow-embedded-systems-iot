---
title: "DevOps/Platform Engineer — Skill Card"
date: 2026-06-20
status: final
tags:
  - skill-card
...
- **Continuous Security Testing in CI:** Integrate automated security testing into the CI/CD pipeline for all code. Tests execute on every push and block merge to main on Critical/High findings: (a) SAST (Static Application Security Testing) — language-specific scanners for C/C++, Python, Go, HCL, YAML, (b) Dependency scanning — all third-party libraries and container base images scanned for known CVEs (Common Vulnerabilities and Exposures), (c) Secret detection — scan all commits and artifacts for hardcoded credentials, API keys, and tokens, (d) Container image scanning — scan built images for vulnerabilities before registry push, (e) IaC (Infrastructure-as-Code) scanning — Terraform, Ansible, and Kubernetes manifests checked for security misconfigurations. Findings feed the Engineering Process Health Dashboard for the shift-left security metric. #continuous-security #shift-left #MR-10
- **Integration test infrastructure provisioning:** Provision and maintain all integration test environments by week 2 of Development: (a) virtualized cloud backend (containerized Backend services + Mosquitto/EMQX broker) for FW↔BACK testing, (b) emulated firmware environment (Renode/QEMU images for each target MCU — Microcontroller Unit) for BACK↔FW and FW↔ML testing, (c) staging OTA (Over-the-Air) pipeline with isolated fleet cohort (separate from production) for MLO↔DEV testing, (d) simulated sensor data generator (configurable data rates, patterns, and fault injection) for DATA↔ML testing, (e) integration test dashboard (shared Grafana) displaying per-contract-pair test status, pass/fail history, and latency trends. #integration-testing #shift-left #HR-5
- **CI/CD integration:** Integrate integration smoke tests into the CI/CD (Continuous Integration / Continuous Deployment) pipeline. Each contract pair's tests run automatically on every push to the pair's shared integration branch. Test failures notify both roles within 15 minutes. #integration-testing #shift-left #HR-5
...
- **Process data source integration:** Expose engineering process data sources to the [[DATA_ENGINEER_SKILL|Data Engineer]] for the Engineering Metrics Pipeline: (a) CI/CD pipeline metrics (build success rate, test pass rate, deployment frequency, pipeline duration) via API or structured log export, (b) Git repository metrics (commit frequency, PR (Pull Request) cycle time, review turnaround) via API, (c) ADR and CCR (Contract Clarification Record) repository access (file-based, Git-hosted) for turnaround time and escalation rate analysis. Data sources must be accessible by week 4 of the first Development cycle. Coordinate with the Data Engineer on schema, access credentials, and refresh frequency. #engineering-metrics #MR-2
...
### 6.11 [[FRONTEND_DASHBOARD_ENGINEER_SKILL|Frontend/Dashboard Engineer]]

- **Provides:** CI/CD pipeline infrastructure for frontend builds (automated build, test, and deployment stages); staging/production environment provisioning with environment parity; deployment rollback capability (automated rollback path tested per release); frontend observability integration (client-side error tracking, Core Web Vitals monitoring, real-time connection metrics); and CDN (Content Delivery Network) / static-asset hosting configuration.
- **Requires:** Frontend build and deployment requirements (build tooling — Vite/Webpack, bundle optimization targets, environment variables); configuration needs per environment (API endpoints, WebSocket/MQTT broker URLs, feature flags); and frontend performance and error budgets for SLO (Service-Level Objective) definition (LCP — Largest Contentful Paint, INP — Interaction to Next Paint, CLS — Cumulative Layout Shift — targets, error-rate thresholds).
- **Cadence:** Initial pipeline setup at project start (CI/CD pipeline configured for the frontend build stack); ongoing coordination for release cycles (Deployment-window alignment, rollback readiness confirmation); pre-release infrastructure health check. #interface-contract #HR-4
