...
**Activities:**
- **Post-launch security monitoring:** Monitor security telemetry (authentication failure rates, anomalous device behavior, certificate expiry, SBOM — Software Bill of Materials — vulnerability advisories) continuously post-launch. Investigate any anomaly exceeding defined thresholds within 1 business hour for Critical, 4 business hours for High. Issue security advisories with remediation SLAs to affected roles as required. #post-launch #field-reliability
- **Security patch coordination:** Issue firmware and platform security advisories to [[FIRMWARE_ENGINEER_SKILL|Firmware]], [[BACKEND_CLOUD_ENGINEER_SKILL|Backend]], [[DEVOPS_PLATFORM_ENGINEER_SKILL|DevOps]], and other affected roles. Define remediation SLAs per severity (Critical — 7 days, High — 30 days) and track completion. Retain release veto authority until the patch is verified deployed. #field-defects
- **Sustaining security baseline review:** Review the security baseline quarterly for post-launch drift (new CVEs — Common Vulnerabilities and Exposures, expired certificates, deprecated cryptographic primitives, supply-chain findings). Issue a Security Baseline Currency Report and initiate remediation for any findings. #sustaining-engineering #lifecycle-gap
- **Incident response participation:** Respond to [[INCIDENT_COMMANDER|Incident Commander]] direction during declared cross-layer incidents within the role's defined response SLA. Provide role-specific expertise to the war room and document any temporary deviations from standard process for retroactive ADR formalization within 5 business days of incident closure. **Note: the [[SECURITY_ENGINEER_SKILL|Security Engineer]]'s release veto is never overridable by the Incident Commander — it remains in full effect during all declared incidents.** Participate in the annual cross-layer incident drill. #cross-layer-incident #incident-commander #emergency-tempo

**Deliverables:**
- Security advisory notices (per finding, on-demand)
- Security Baseline Currency Report (quarterly)
- Patch remediation tracking log (continuous)
