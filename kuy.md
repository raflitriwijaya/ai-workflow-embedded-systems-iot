...
> **Purpose:** Master reference for AI agents (Claude Code) in this project. Read in full before accessing any role SKILL.md or producing any artifact. Supersedes default Claude Code behavior on naming, format, governance, and agent authority.
...
**Vault:** Obsidian-compatible Markdown with YAML frontmatter, `[[wikilinks]]`, `#tags`;
...
4. **Never silently deviate:** any infeasibility (unmeetable budget, unimplementable contract, contradictory requirement) is raised as an ADR or CCR with measured evidence; halt that thread and notify consumers — do not proceed, approximate, or work around.
...
[NOT YET AVAILABLE — fill in manually: ADR storage location (`docs/decisions/`? — registry referenced in schema but directory not yet created), ADR numbering convention, the approved technology stack that defines "outside the stack" for ADR trigger purposes.]
...
[NOT YET AVAILABLE — fill in manually: ARB quorum composition, member list, meeting cadence, from `EMBEDDED_SYSTEMS_ARCHITECT_SKILL.md` §7.Z.]
...
[NOT YET AVAILABLE — fill in manually the quantitative scalability targets (fleet size, MQTT throughput, data ingestion rate/s, OTA concurrency, cloud API p99 latency) from `EMBEDDED_SYSTEMS_ARCHITECT_SKILL.md` §5.]
