...
**Scope:** 71 of 91 contracts converted (the agent-coordination-critical surface, the cross-layer planning/feasibility edges, the operational/security/release edges, and the strategic business + research-interface edges), across five batches. The remaining 20 are tracked as `[NOT YET CONVERTED]` and remain authoritative as §6 prose in their owning `*_SKILL.md`. Each JSON Schema carries an `x-macp` block holding `decision_tier`, `cadence`, and the `confirm_conditions` / `reject_conditions` / `counter_conditions` validation rules.
...
> **Batch 1** = rows 1–15 (`FW↔BACK-001` … `PO↔ALL-RELEASE-001`); **Batch 2** = rows 16–29 (`DATA↔BACK-INGEST-001` … `ARCH↔PO-001`); **Batch 3** = rows 30–44 (`ARCH↔HW-001` … `BACK↔SEC-001`); **Batch 4** = rows 45–58 (`DEVOPS↔QA-001` … `QA↔BIZ-001`); **Batch 5** = rows 59–71 (`ARCH↔BIZ-001` … `RES↔MLOPS-001`). 71 of 91 contracts converted.
...
- 20 of 91 contracts `[NOT YET CONVERTED]` — convert as agent-coordination demand warrants; preserve canonical role ordering and 3-digit sequence numbers.
